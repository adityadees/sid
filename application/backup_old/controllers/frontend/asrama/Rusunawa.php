...
class Rusunawa extends MY_Controller{
	...
	public function index(){
		...
		if (isset($this->data['token_mhs'])) {
			...
			$this->data['regsuc'] = $this->db->query("SELECT * FROM registrasi where registrasi_validasi='yes' and penghuni_kpm='$kpm'")->result();
			
			$kuotapria = 0;
			$kuotaputri = 0;
			$kuotabaruputri = 0;
			$filter = $this->db->query("SELECT * FROM registrasi INNER JOIN penghuni on registrasi.penghuni_kpm=penghuni.penghuni_kpm where registrasi_asrama='rusunawa' && registrasi_validasi != 'yes'")->result();
			foreach ($filter as $flt) {
				if($flt->penghuni_jk=='L'){
					$kuotapria +=1;
				} else if ($flt->penghuni_jk=='P') {
					$kuotaputri +=1;
					$kuotabaruputri +=1;
				} else {}
			}
			if($q[0]->penghuni_jk == 'L'){
				$kuota = $kuotapria;
			}
			if($q[0]->penghuni_jk == 'P'){
				$kuota = $kuotaputri;
			}
		}
		...
		$this->data['kuota'] = (204-$isi-$rusak-$kuota);
		$this->data['svgfile'] = 'rusunawa_lama_1.svg';
...
//		|| $_FILES['filestruk']['name'] == '' 
		if($_FILES['filespp']['name'] == ''){
			$this->session->set_flashdata('error', 'Lampiran tidak boleh kosong');
			redirect('asrama/rusunawa');
		} else {

			$upspp = $this->go_upload('filespp', 'uploads/asrama/rusunawa/'.$kpm, 'jpeg|jpg|png|pdf', TRUE);
			if($upspp['status'] != 'OK'){
				...
				$dregistrasi = [ 
					...
					'registrasi_tgl_end'	=> date('2019-08-03'),
				...
				/*$upstruk = $this->go_upload('filestruk', 'uploads/asrama/rusunawa/'.$kpm, 'jpeg|jpg|png|pdf', TRUE);
